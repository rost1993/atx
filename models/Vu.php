	...
	function rendering_list($post) {
		...
		if(count($data) == 0) {
			...
			for($i = 0, $k1 = 1, $k2 = 1; $i < count($data); $i++) {
				...
				if($role >= 2) {
				   $temp .= "<td " . $style_border . "><button type='button' id='btnEditItem' data-item='1' data-nsyst='" . $data[$i]['id'] . "' class='btn btn-sm btn-info' title='Изменить водительское удостоверение'><span class='fa fa-pencil'>&nbsp</span>Изменить</button></td>"
