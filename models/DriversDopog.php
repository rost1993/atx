...
class DriversDopog extends Model {
				...
				if($data[$i]['ibd_arx'] == 1) {
					$html .= "<tr>"
							...
							if(($role > 1) && ($role != 4)) {
								...
								. "<button type='button' class='btn btn-sm btn-info' id='btnEditItem' data-nsyst='" . $data[$i]['id'] . "' data-item='15'><span class='fa fa-pencil'>&nbsp</span>Изменить</button></td>"
								...
								. "<div class='dropdown-menu' aria-labelledby='btnDropdownRemove'>"
									. "<button type='button' class='dropdown-item' id='btnRemoveItem' data-nsyst='" . $data[$i]['id'] . "' data-item='15' data-object='" . $data[$i]['id_driver'] . "'><span class='fa fa-check text-success'>&nbsp</span>Подтверждаю удаление</button></div></div>"
							. "</td>";
							} else {
	...
	public function rendering_window($post) {
		...
		$number_dopog = $date_start_dopog = $date_end_dopog = $path_to_file = $file_extension = "";
		if($id != -1) {
		...
		$html = "<div class='col-12'>"
				...
				. "<div class='form-row'>"
					...
					. "<div class='col-4 mb-1 text-left'>"
						. "<div id='uploadFileContainer'>" . Functions::rendering_icon_file($path_to_file, $file_extension, $id, 17, true) . "</div>"
					. "</div>"
